...
const BellmanFordComponent = () => {
  ...
  const startBellmanFord = () => {
    ...
    const found = bellmanFord.execute();
    
    if (found) {
      bellmanFord.paintShortestPath();
      setIsPathFound(true);
    }

    // A timer to reset the visualizing state after the animation is done
    const totalDelay = (bellmanFord.dist[end.current.x][end.current.y] + 5) * DELAY_NORMAL;
    ...
    <div className="visualization-container">
      <div className="visualization-content">
        <div className="header">
          <h1 className="text-2xl font-bold">Bellman-Ford Algorithm</h1>
          <button
            onClick={() => navigate('/')}
        ...
        <div className="controls">
          ...
          >
            {isVisualizing ? "Visualizing..." : "Start Bellman-Ford"}
          </button>
        </div>
